...
export default class SpeechTask extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      speechTaskStates: [
        [
          'start',
          'dot-white',
          i18next.t('speechTask.header.start'),
        ...
        [
          'introTask1',
          'dot-white',
          i18next.t('speechTask.header.state_prepare'),
          i18next.t('speechTask.question_0.header'),
        ...
        [
          'runTask1',
          'dot-red',
          i18next.t('speechTask.header.state_run'),
          i18next.t('speechTask.question_0.header'),
          i18next.t('speechTask.feedback_0') +
            ' ' +
            i18next.t('speechTask.question_0.question'),
    ...
    this.incrementSpeechTaskStateCounter =
      this.incrementSpeechTaskStateCounter.bind(this);
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    if (
      prevState.speechTaskStateCounter !== this.state.speechTaskStateCounter
    ) {
      ...
      this.setState((state) => {
        return {
          remainingMilliseconds:
            state.startMilliseconds - (Date.now() - state.startTime),
        };
      });
    }, 1);
    if (
      ['introTask1', 'introTask2', 'introTask3'].includes(
        this.state.speechTaskStates[this.state.speechTaskStateCounter][0],
      )
    ) {
      ...
      this.timeoutID = setTimeout(() => {
        clearInterval(this.intervalID);
        if (
          this.state.speechTaskStateCounter ===
          this.state.speechTaskStates.length - 1
        ) {
  ...
  setVolume(currentVolume) {
    ...
    if (['runTask1'].includes(currentState)) {
      ...
      let allTicks =
        this.state.speakBreakCounterQ1 + this.state.speakingTickCounterQ1;
      if (currentVolume > 0) {
        this.setState({
          audioMeanQ1:
            (this.state.audioMeanQ1 * allTicks + currentVolume) /
            (allTicks + 1),
        });
      }
      ...
      // we only want to adjust audioMean if a signal is given
      let allTicks =
        this.state.speakBreakCounterQ2 + this.state.speakingTickCounterQ2;
      if (currentVolume > 0) {
        this.setState({
          audioMeanQ2:
            (this.state.audioMeanQ2 * allTicks + currentVolume) /
            (allTicks + 1),
        });
      }
      ...
      // we only want to adjust audioMean if a signal is given
      let allTicks =
        this.state.speakBreakCounterQ3 + this.state.speakingTickCounterQ3;
      if (currentVolume > 0) {
        this.setState({
          audioMeanQ3:
            (this.state.audioMeanQ3 * allTicks + currentVolume) /
            (allTicks + 1),
        });
      }
  ...
  render() {
    ...
    return this.state.speechTaskStateCounter < 2 ? (
