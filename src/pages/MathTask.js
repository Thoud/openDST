...
export default class MathTask extends React.Component {
  ...
  handleNumberInput(event) {
    if (this.state.currentFeedback === 'default') {
      ...
      if (tempNumberInput === this.getCurrentQuestion().answer.toString()) {
        ...
        tempNumberInput.length === lengthOfAnswer() &&
        tempNumberInput !== this.getCurrentQuestion().answer.toString()
      ) {
        this.setState({
          numberInput: tempNumberInput,
  ...
  updateTimer() {
    if (!this.props.cancelDialogIsOpen) {
      if (this.mathTask_seconds > this.MATHTASK_TOTAL_SECONDS) {
        this.props.endMathTask(
          Math.round(
            (this.state.correctAnswerCounter / this.state.questionCounter) *
              100,
          ),
        );
        ...
        if (
          taskElapsedTime <
          this.state.questionDuration + this.state.timePaused
        ) {
          this.timeoutProgressBarID = setTimeout(() => {
            this.setState({
              elapsedTimeCurrentQuestion:
                taskElapsedTime - this.state.timePaused,
            });
            this.updateTimer();
          }, 10);
        } else {
  ...
  questionCountDown() {
    if (this.mathTask_seconds > this.MATHTASK_TOTAL_SECONDS) {
      this.props.endMathTask(
        Math.round(
          (this.state.correctAnswerCounter / this.state.questionCounter) * 100,
        ),
      );
    } else if (this.state.currentFeedback === 'default') {
      ...
      this.props.updateMathTaskPerformance(
        isAnswerCorrect,
        this.noAnswerStreak,
        this.state.elapsedTimeCurrentQuestion / 1000,
        this.state.timePaused,
        this.state.questionDuration,
        this.currentQuestionIndex,
        (this.state.taskBeginTime - this.state.testStartTimestamp) / 1000,
        (Date.now() - this.state.testStartTimestamp) / 1000,
        this.state.taskList,
  ...
  generateTaskQuestions(numberOfTasks) {
    ...
    while (taskList.length < numberOfTasks) {
      ...
      switch (randomOperator) {
        case '/':
          ...
          let random = Math.floor(
            Math.random() * divisorOfMathTermCounter.length,
          );
      ...
      if (
        mathQuestion.answer !== undefined &&
        mathQuestion.answer !== 0 &&
        mathQuestion.answer < 100
      ) {
    ...
    return (
      <div>
        ...
        <section className="taskSection">
          <Progressbar
            questionCurrentInPercentage={Math.round(
              (this.state.elapsedTimeCurrentQuestion /
                this.state.questionDuration) *
                100,
            )}
            studyPage="mathTask"
