...
export default function StepperWithLabels(props) {
  function getStepsAndCounter(slideSequences, studyPage) {
    ...
    if (
      props.studyPagesSequence.indexOf('mathTaskTutorial') <
      props.studyPagesSequence.indexOf('speechTaskTutorial')
    ) {
      if (studyPage === 'introduction' || studyPage === 'mathTaskTutorial') {
        for (let i = 0; i < slideSequences[studyPage].length; i++) {
          list.push(
            i18next.t(`stepper.${studyPage}.${slideSequences[studyPage][i]}`),
          );
        }
      } else if (
        studyPage === 'mathTaskResult' ||
        studyPage === 'speechTaskTutorial'
      ) {
        for (let i = 0; i < slideSequences['mathTaskResult'].length; i++) {
          list.push(
            i18next.t(
              `stepper.mathTaskResult.${slideSequences['mathTaskResult'][i]}`,
            ),
          );
        }
        for (let i = 0; i < slideSequences['speechTaskTutorial'].length; i++) {
          list.push(
            i18next.t(
              `stepper.speechTaskTutorial.${slideSequences['speechTaskTutorial'][i]}`,
            ),
          );
        }
      ...
      if (studyPage === 'introduction' || studyPage === 'speechTaskTutorial') {
        for (let i = 0; i < slideSequences['introduction'].length; i++) {
          list.push(
            i18next.t(
              `stepper.introduction.${slideSequences['introduction'][i]}`,
            ),
          );
        }
        for (let i = 0; i < slideSequences['speechTaskTutorial'].length; i++) {
        ...
        for (let i = 0; i < slideSequences['mathTaskTutorial'].length; i++) {
          list.push(
            i18next.t(
              `stepper.mathTaskTutorial.${slideSequences['mathTaskTutorial'][i]}`,
            ),
          );
        }
