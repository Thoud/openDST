...
class TrafficLightComponent extends Component {
  constructor(props) {
    ...
    this.chooseImageAfterDifferentTime =
      this.chooseImageAfterDifferentTime.bind(this);
  }

  componentDidMount() {
    this.interval = setInterval(
      () =>
        this.setState({ color_state: this.chooseImageAfterDifferentTime() }),
      100,
    );
  }
  ...
  chooseImage() {
    ...
    if (want_color === 0) {
      ...
      want_color === 1 &&
      (this.state.color_state === 0 ||
        this.state.color_state === 1 ||
        this.state.color_state === 2)
    ) {
      show_color = 1;
    } else if (
      want_color === 2 ||
      (want_color === 1 && this.state.color_state === 3) ||
      (want_color === 3 && this.state.color_state === 1)
    ) {
      show_color = 2;
    } else {
      show_color = 3;
